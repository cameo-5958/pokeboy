#!/bin/bash
# Build the FAT disk that stands in for the SD card's boot partition: the
# AI-patched ROM and the exported int8 weights, which the image expects at
# /boot. Built with Buildroot's own mtools, so no loop mount and no root.
#
#   ROM=... WEIGHTS=... make-data-disk.sh
#
# Result: $WORK/emu/data.vfat
set -euo pipefail
