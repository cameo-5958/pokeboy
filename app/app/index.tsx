...
// binary at build time. "Pull latest" cannot change it.
...
const EmulatorContext = createContext<{
  ...
  webViewRef: { current: EmulatorHandle | null };
...
export default function EmulatorScreen() {
  ...
  const webViewRef = useRef<EmulatorHandle | null>(null);
  ...
  const onMessage = useCallback((event: EmulatorMessageEvent) => {
  ...
  const emulator = useMemo(
    () => {
      ...
      return {
        ...
        // Native pre-injects; web posts `config` on request. Same values, so
        // embed.html sees an identical PokeboyRuntime either way.
    ...
    <View style={[styles.lcd, { width, height, borderRadius: u(4), borderWidth: u(1.5) }]}>
      {emulator ? (
        <EmulatorSurface
          ...
          ref={webViewRef}
          uri={emulator.uri}
          injected={emulator.injected}
          config={emulator.config}
          readAccessUri={emulator.readAccessUri}
